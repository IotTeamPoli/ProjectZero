...
function FreeboardModel(a, b, c) {
    ...
    this.version = 0, this.isEditing = ko.observable(!1), this.allow_edit = ko.observable(!1), this.allow_edit.subscribe(function(a) {
        ...
        this.saveDashboard = function() {
            freeboard.showDialog("Would you like to replace default dashboard with the current one?", "Set Default", "Replace", "Cancel",
                function() {
                    $.post("saveDashboard", {
                        json_string: JSON.stringify(d.serialize())
                    })
                })
        },

        this.setDefault = function() {
            freeboard.showDialog("Would you like to replace this dashboard with the default one?", "Set Default", "Replace", "Cancel",
                function() {
                    $.post("saveDashboard", {
                        json_string: "{\"version\":1,\"allow_edit\":true,\"plugins\":[],\"panes\":[],\"datasources\":[],\"columns\":3}"
                    })
                })
        },
    ...
    function a(a) {
        ...
        a && (c = function() {
            var a = this,
                c = ko.dataFor(a),
                d = q(c);
            $(a).attr("data-sizex", Math.min(c.col_width(), b, r.cols)).attr("data-row", d.row).attr("data-col", d.col), c.processSizeChange()
...
function PaneModel(a, b) {
    var c = this;
    this.title = ko.observable(), this.width = ko.observable(1), this.row = {}, this.col = {}, this.col_width = ko.observable(1), this.col_width.subscribe(function() {
        c.processSizeChange()
        ...
        if (d.datasourceRefreshNotifications = {}, d.calculatedSettingScripts = {}, !_.isUndefined(d.type())) {
            ...
            _.each(a, function(a) {
                if ("calculated" == a.type) {
                    var b = e[a.name];
                    if (!_.isUndefined(b)) {
                        (b.match(/;/g) || []).length <= 1 && -1 == b.indexOf("return") && (b = "return " + b);
                        var f;
                        try {
...
DatasourceModel = function(a, b) {
        ...
        function b() {
            function b(a) {
                var b = $("<tr></tr>"),
                    ...
                    f = $('<li><i class="icon-trash icon-white"></i></li>').click(function() {
        ...
        function f(f, g, h, i, j) {
            function k(a, b) {
                var c = $('<div id="setting-row-' + a + '" class="form-row"></div>').appendTo(o);
            ...
            function l(c) {
                _.each(c, function(c) {
                    function d() {
                        n.settings[c.name].length > 0 ? l.show() : l.hide()
                    }

                    function e(a) {
                        var b = $("<tr></tr>").appendTo(o),
                            e = {};
                        _.isArray(n.settings[c.name]) || (n.settings[c.name] = []), n.settings[c.name].push(e), _.each(c.settings, function(c) {
                            var d = $("<td></td>").appendTo(b),
                                f = "";
                            _.isUndefined(a[c.name]) || (f = a[c.name]), e[c.name] = f, $('<input class="table-row-value" type="text">').appendTo(d).val(f).change(function() {
                                e[c.name] = $(this).val()
                            })
                        }), b.append($('<td class="table-row-operation"></td>').append($('<ul class="board-toolbar"></ul>').append($("<li></li>").append($('<i class="icon-trash icon-white"></i>').click(function() {
                            var a = n.settings[c.name].indexOf(e); - 1 != a && (n.settings[c.name].splice(a, 1), b.remove(), d())
                        }))))), h.scrollTop(h[0].scrollHeight), d()
                    }!_.isUndefined(c.default_value) && _.isUndefined(i[c.name]) && (i[c.name] = c.default_value);
                    var f = c.name;
                    _.isUndefined(c.display_name) || (f = c.display_name);
                    var g = k(c.name, f);
                    switch (c.type) {
                        case "array":
                            var h = $('<div class="form-table-value-subtable"></div>').appendTo(g),
                                j = $('<table class="table table-condensed sub-table"></table>').appendTo(h),
                                l = $("<thead></thead>").hide().appendTo(j),
                                m = $("<tr></tr>").appendTo(l),
                                o = $("<tbody></tbody>").appendTo(j),
                                p = [];
                            _.each(c.settings, function(a) {
                                var b = a.name;
                                _.isUndefined(a.display_name) || (b = a.display_name), $("<th>" + b + "</th>").appendTo(m)
                            }), c.name in i && (p = i[c.name]), $('<div class="table-operation text-button">ADD</div>').appendTo(g).click(function() {
                                var a = {};
                                _.each(c.settings, function(b) {
                                    a[b.name] = ""
                                }), e(a)
                            }), _.each(p, function(a) {
                                e(a)
                            ...
                            break;
                        case "boolean":
                            n.settings[c.name] = i[c.name];
                            var q = $('<div class="onoffswitch"><label class="onoffswitch-label" for="' + c.name + '-onoff"><div class="onoffswitch-inner"><span class="on">YES</span><span class="off">NO</span></div><div class="onoffswitch-switch"></div></label></div>').appendTo(g),
                                r = $('<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="' + c.name + '-onoff">').prependTo(q).change(function() {
                                    n.settings[c.name] = this.checked
                                });
                            c.name in i && r.prop("checked", i[c.name]);
                            break;
                        case "option":
                            var s = i[c.name],
                                r = $("<select></select>").appendTo($('<div class="styled-select"></div>').appendTo(g)).change(function() {
                                    n.settings[c.name] = $(this).val()
                                });
                            _.each(c.options, function(a) {
                                var b, c;
                                _.isObject(a) ? (b = a.name, c = a.value) : b = a, _.isUndefined(c) && (c = b), _.isUndefined(s) && (s = c), $("<option></option>").text(b).attr("value", c).appendTo(r)
                            }), n.settings[c.name] = s, c.name in i && r.val(i[c.name]);
                            break;
                        default:
                            if (n.settings[c.name] = i[c.name], "calculated" == c.type) {
                                var r = $("<textarea></textarea>").appendTo(g).change(function() {
                                    n.settings[c.name] = $(this).val()
                                });
                                c.name in i && r.val(i[c.name]), b.createValueEditor(r);
                                var t = $('<ul class="board-toolbar datasource-input-suffix"></ul>'),
                                    u = $('<li><i class="icon-plus icon-white"></i><label>DATASOURCE</label></li>').mousedown(function(a) {
                                        a.preventDefault(), $(r).focus(), $(r).insertAtCaret('datasources["'), $(r).trigger("freeboard-eval")
                                    }),
                                    v = $('<li><i class="icon-fullscreen icon-white"></i><label>.JS EDITOR</label></li>').mousedown(function(b) {
                                        b.preventDefault(), a.displayJSEditor(r.val(), function(a) {
                                            r.val(a), r.change()
                                        })
                                    });
                                $(g).append(t.append([u, v]))
                            } else {
                                var r = $('<input type="text">').appendTo(g).change(function() {
                                    n.settings[c.name] = "number" == c.type ? Number($(this).val()) : $(this).val()
                                });
                                c.name in i && r.val(i[c.name])
                            }
                    }
                    _.isUndefined(c.suffix) || g.append($('<div class="input-suffix">' + c.suffix + "</div>")), _.isUndefined(c.description) || g.append($('<div class="setting-description">' + c.description + "</div>"))
                })
            }
            var m, n = {
                    type: h,
                ...
                o = $("<div></div>"),
                p = $('<div id="plugin-description"></div>').hide();
            o.append(p), new DialogBox(o, f, "Save", "Cancel", function() {
                $(".validation-error").remove();
                for (var a = 0; a < m.settings.length; a++) {
                    var b = m.settings[a];
                    if (b.required && (_.isUndefined(n.settings[b.name]) || "" == n.settings[b.name])) return c(b.name, "This is required."), !0;
                    if ("number" == b.type && !e(n.settings[b.name])) return c(b.name, "Must be a number."), !0
                }
                _.isFunction(j) && j(n)
            });
            var q, r = _.keys(g);
            if (r.length > 1) {
                var s = k("plugin-types", "Type");
                q = $("<select></select>").appendTo($('<div class="styled-select"></div>').appendTo(s)), q.append($("<option>Select a type...</option>").attr("value", "undefined")), _.each(g, function(a) {
                    q.append($("<option></option>").text(a.display_name).attr("value", a.type_name))
                }), q.change(function() {
                    n.type = $(this).val(), n.settings = {}, d(), m = g[q.val()], _.isUndefined(m) ? ($("#setting-row-instance-name").hide(), $("#dialog-ok").hide()) : ($("#setting-row-instance-name").show(), m.description && m.description.length > 0 ? p.html(m.description).show() : p.hide(), $("#dialog-ok").show(), l(m.settings))
                })
            } else 1 == r.length && (m = g[r[0]], n.type = m.type_name, n.settings = {}, l(m.settings));
            q && (_.isUndefined(h) ? ($("#setting-row-instance-name").hide(), $("#dialog-ok").hide()) : ($("#dialog-ok").show(), q.val(h).trigger("change")))
        }
        return {
            createPluginEditor: function(a, b, c, d, e, g) {
                f(a, b, c, d, e, g)
            }
        }
        ...
        function c(a, b) {
            var c, d = f.exec(a),
                h = [];
            if (d)
                if ("" == d[1]) _.each(b, function(a) {
                    h.push({
                        value: a.name(),
                        follow_char: '"]["'
                    })
                });
                else if ("" != d[1] && _.isUndefined(d[2])) c = d[1], _.each(b, function(a) {
                var b = a.name();
                b != c && 0 == b.indexOf(c) && h.push({
                    value: b,
                    follow_char: '"]["'
                })
            });
            else {
                var i = _.find(b, function(a) {
                    return a.name() === d[1]
                });
                if (!_.isUndefined(i)) {
                    var j = "";
                    _.isUndefined(d[2]) || (j = d[2] + d[3]);
                    var k = j.split('"]["');
                    j = "data";
                    for (var l = 1; l < k.length - 1; l++) "" != k[l] && (dataPathItem = '["' + k[l] + '"]', j += dataPathItem);
                    var m = _.last(k);
                    '"' == m.charAt(m.length - 1) && (m = m.replace(/\[\"?$/, ""), j = j + '["' + m + '"]');
                    var n = i.getDataRepresentation(j);
                    if (_.isArray(n))
                        for (var l = 0; l < n.length; l++) {
                            var o = '"]';
                            _.isObject(n[l]) ? o += '"]["' : _.isArray(n[l]) && (o += '"]['), h.push({
                                value: l,
                                follow_char: o
                            })
                        } else _.isObject(n) && (c = m, -1 == _.keys(n).indexOf(c) && _.each(n, function(a, b) {
                            if (b != m && 0 == b.indexOf(m)) {
                                var c = '"]';
                                _.isArray(a) ? c = '"][' : _.isObject(a) && (c = '"]["'), h.push({
                                    value: b,
                                    follow_char: c
                                })
                            }
        ...
        function d(d) {
            var f = null,
                h = 0;
            $(d).addClass("calculated-value-input").bind("keyup mouseup freeboard-eval", function(b) {
                if (f && "keyup" == b.type && (38 == b.keyCode || 40 == b.keyCode || 13 == b.keyCode)) return void b.preventDefault();
                var i = $(d).val().substring(0, $(d).getCaretPosition());
                if (i = i.replace(String.fromCharCode(160), " "), c(i, a.datasources()), g.length > 0) {
                    f || (f = $('<ul id="value-selector" class="value-dropdown"></ul>').insertAfter(d).width($(d).outerWidth() - 2).css("left", $(d).position().left).css("top", $(d).position().top + $(d).outerHeight() - 1)), f.empty(), f.scrollTop(0);
                    var j = !0;
                    h = 0;
                    var k = 0;
                    _.each(g, function(a) {
                        var b = $("<li>" + a.value + "</li>").appendTo(f).mouseenter(function() {
                            $(this).trigger("freeboard-select")
                        }).mousedown(function(a) {
                            $(this).trigger("freeboard-insertValue"), a.preventDefault()
                        }).data("freeboard-optionIndex", k).data("freeboard-optionValue", a.value).bind("freeboard-insertValue", function() {
                            var b = a.value;
                            if (_.isUndefined(a.follow_char) || (b += a.follow_char), _.isUndefined(e)) $(d).insertAtCaret(b);
                            else {
                                var c = i.lastIndexOf(e); - 1 != c && $(d).replaceTextAt(c, c + e.length, b)
                            }
                            $(d).triggerHandler("mouseup")
                        }).bind("freeboard-select", function() {
                            $(this).parent().find("li.selected").removeClass("selected"), $(this).addClass("selected"), h = $(this).data("freeboard-optionIndex")
                        });
                        j && ($(b).addClass("selected"), j = !1), k++
                    })
                } else $(d).next("ul#value-selector").remove(), f = null, h = -1
            }).focus(function() {
                $(d).css({
                    "z-index": 3001
                }), b(d)
                ...
                $(d).css({
                    height: "",
                    "z-index": 3e3
                }), $(d).next("ul#value-selector").remove(), f = null, h = -1
            }).bind("keydown", function(a) {
                if (f)
                    if (38 == a.keyCode || 40 == a.keyCode) {
                        a.preventDefault();
                        var b = $(f).find("li");
                        38 == a.keyCode ? h-- : 40 == a.keyCode && h++, 0 > h ? h = b.size() - 1 : h >= b.size() && (h = 0);
                        var c = $(b).eq(h);
                        c.trigger("freeboard-select"), $(f).scrollTop($(c).position().top)
                    } else 13 == a.keyCode && (a.preventDefault(), -1 != h && $(f).find("li").eq(h).trigger("freeboard-insertValue"))
            })
        }
        var e, f = new RegExp('.*datasources\\["([^"]*)("\\]\\[")?(.*)$'),
            g = [];
        return {
            createValueEditor: function(a) {
                d(a)
            }
        }
    },
...
var freeboard = function() {
    ...
    return ko.bindingHandlers.pluginEditor = {
        init: function(a, f, g, i) {
            var j = ko.unwrap(f()),
                k = {},
                l = void 0,
                m = "";
            "datasource" == j.type ? (k = b, m = "Datasource") : "widget" == j.type ? (k = c, m = "Widget") : "pane" == j.type && (m = "Pane"), $(a).click(function() {
                if ("delete" == j.operation) {
                    var f = $("<p>Are you sure you want to delete this " + m + "?</p>");
                    new DialogBox(f, "Confirm Delete", "Yes", "No", function() {
                        "datasource" == j.type ? e.deleteDatasource(i) : "widget" == j.type ? e.deleteWidget(i) : "pane" == j.type && e.deletePane(i)
                    })
                } else {
                    var g = void 0;
                    "datasource" == j.type ? "add" == j.operation ? l = {} : (g = i.type(), l = i.settings(), l.name = i.name()) : "widget" == j.type ? "add" == j.operation ? l = {} : (g = i.type(), l = i.settings()) : "pane" == j.type && (l = {}, "edit" == j.operation && (l.title = i.title(), l.col_width = i.col_width()), k = {
                        settings: {
                            settings: [{
                                name: "title",
                                display_name: "Title",
                    ...
                    }), h.createPluginEditor(m, k, g, l, function(f) {
                        if ("add" == j.operation) {
                            if ("datasource" == j.type) {
                            ...
                            } else if ("widget" == j.type) {
                                var g = new WidgetModel(e, c);
                                g.settings(f.settings), g.type(f.type), i.widgets.push(g), d.attachWidgetEditIcons(a)
                            }
                        } else "edit" == j.operation && ("pane" == j.type ? (i.title(f.settings.title), i.col_width(f.settings.col_width), d.processResize(!1)) : ("datasource" == j.type && (i.name(f.settings.name), delete f.settings.name), i.type(f.type), i.settings(f.settings)))
                    })
                }
            })
        }
...
$.extend(freeboard, jQuery.eventEmitter),
    ...
    function() {
        ...
        function b(a, b) {
            var d = $(a).data().values;
            d || (d = []), d.length >= c && d.shift(), d.push(Number(b)), $(a).data().values = d, $(a).sparkline(d, {
                type: "line",
                height: "100%",
                width: "100%",
                fillColor: !1,
        ...
        var e = function(c) {
            ...
            this.render = function(a) {
                ...
                d()
            }, this.onCalculatedValueChanged = function(c, d) {
                "value" == c && (e.animate ? a(d, h, 500) : h.text(d), e.sparkline && b(j, d))
            }, this.onDispose = function() {}, this.getHeight = function() {
                return "big" == e.size || e.sparkline ? 2 : 1
            }, this.onSettingsChanged(c)
        };
        ...
        var g = function(a) {
            ...
            var c, d = "gauge-" + f++,
                ...
                g = $('<div class="gauge-widget" id="' + d + '"></div>'),
                h = !1,
                i = a;
            this.render = function(a) {
                h = !0, $(a).append(e).append($('<div class="gauge-widget-wrapper"></div>').append(g)), b()
            }, this.onSettingsChanged = function(a) {
        ...
        var h = function(a) {
            var c = $('<h2 class="section-title"></h2>'),
                d = $('<div class="sparkline"></div>');
            this.render = function(a) {
                $(a).append(c).append(d)
            }, this.onSettingsChanged = function(a) {
                c.html(_.isUndefined(a.title) ? "" : a.title)
        ...
        var l = function(a) {
            ...
            this.render = function(a) {
                function e() {
                    ...
                    }), google.maps.event.addDomListener(a, "mouseleave", function() {
